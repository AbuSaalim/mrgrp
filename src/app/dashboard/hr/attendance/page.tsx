...
export default function LiveAttendancePage() {
  ...
  const getStatusTheme = (status: string) => {
    switch(status) {
      case "Present": return { wrapper: "border-emerald-500/30 dark:border-emerald-500/40 hover:border-emerald-500/80 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.05)]", badge: "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20 shadow-sm dark:shadow-none", icon: "text-emerald-600 dark:text-emerald-400" };
      case "Absent":
      case "Missed Out": return { wrapper: "border-rose-500/30 dark:border-rose-500/40 hover:border-rose-500/80 hover:bg-rose-50 dark:hover:bg-rose-500/10 shadow-[0_0_15px_rgba(244,63,94,0.05)]", badge: "bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-500/20 shadow-sm dark:shadow-none", icon: "text-rose-600 dark:text-rose-400" };
      case "In Progress": return { wrapper: "border-blue-500/30 dark:border-blue-500/40 hover:border-blue-500/80 hover:bg-blue-50 dark:hover:bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.05)]", badge: "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-500/20 shadow-sm dark:shadow-none", icon: "text-blue-600 dark:text-blue-400" };
      default: return { wrapper: "border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800/50", badge: "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700", icon: "text-slate-500 dark:text-slate-400" };
    }
  };
  ...
  return (
    <div className="space-y-6 text-slate-800 dark:text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="relative flex items-center bg-white/80 dark:bg-slate-900/60 p-2.5 px-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-[0_0_15px_rgba(0,0,0,0.2)] w-fit self-start md:self-auto hover:border-blue-500/50 transition-colors group z-40 cursor-pointer backdrop-blur-md">
          ...
          <DatePicker
              ...
              <div className="p-3 sm:p-4 pb-2 bg-white dark:bg-[#0B1121] border-b border-slate-200 dark:border-slate-800/50">
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={decreaseMonth} disabled={prevMonthButtonDisabled} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white disabled:opacity-30 transition-colors"><ChevronLeft className="w-5 h-5" /></button>
                    <span className="text-base sm:text-lg font-bold text-slate-900 dark:text-white tracking-wide">{MONTHS[date.getMonth()]} {date.getFullYear()}</span>
                  </div>
                  <button type="button" onClick={increaseMonth} disabled={nextMonthButtonDisabled} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white disabled:opacity-30 transition-colors"><ChevronRight className="w-5 h-5" /></button>
                </div>
                <div className="flex items-center justify-center gap-2">
                  <div className="relative">
                    <select value={MONTHS[date.getMonth()]} onChange={({ target: { value } }) => changeMonth(MONTHS.indexOf(value))} className="appearance-none bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 sm:px-3.5 py-1.5 pr-8 text-xs font-bold text-slate-800 dark:text-slate-200 outline-none cursor-pointer hover:border-blue-500 transition-colors">
                      {MONTHS.map((option) => (<option key={option} value={option}>{option}</option>))}
                    </select>
                    <ChevronDown className="w-3.5 h-3.5 text-slate-500 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                  </div>
                  <div className="relative">
                    <select value={date.getFullYear()} onChange={({ target: { value } }) => changeYear(Number(value))} className="appearance-none bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 sm:px-3.5 py-1.5 pr-8 text-xs font-bold text-slate-800 dark:text-slate-200 outline-none cursor-pointer hover:border-blue-500 transition-colors">
            ...
            className="bg-transparent border-none outline-none text-sm font-bold text-slate-800 dark:text-slate-200 w-24 cursor-pointer placeholder:text-slate-500"
      ...
      {isLoading ? (
        <div className="text-center py-10 text-blue-400/70 animate-pulse bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-800 text-sm font-bold tracking-widest uppercase shadow-sm">
          Syncing records...
        </div>
      ) : (
        <div className="w-full relative z-10">
          ...
          {/* 💻 DESKTOP VIEW */}
          <div className="hidden md:block bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden">
                  ...
                  return (
                    <tr key={att._id} className="hover:bg-slate-100 dark:hover:bg-slate-800/40 transition-colors group">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-1">{att.userId?.role}</div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {att.punchIn?.time ? (
                          <div>
                            <div className="font-bold text-slate-900 dark:text-white tracking-wide">{new Date(att.punchIn.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                      ...
                      <td className="px-6 py-4">
                        {att.punchOut?.time ? (
                          <div>
                            <div className="font-bold text-slate-900 dark:text-white tracking-wide">{new Date(att.punchOut.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                            <div className="flex items-start mt-1.5 gap-1.5 text-[11px] text-slate-500 dark:text-slate-400">
                              <MapPin className="h-3 w-3 mt-0.5 text-blue-500 flex-shrink-0" />
                      ...
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => openEditModal(att)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-blue-500/20 hover:border-blue-500/50 hover:text-blue-600 dark:hover:text-blue-400 text-slate-600 dark:text-slate-300 text-xs font-bold rounded-xl flex items-center justify-center ml-auto transition-all shadow-sm">
          ...
          <div className="md:hidden space-y-4">
            {attendances.length === 0 ? (
              <div className="text-center py-10 text-slate-500 bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-800 font-medium">No records found.</div>
            ) : attendances.map((att) => {
              const theme = getStatusTheme(att.status);
              return (
                <div key={att._id} className={`bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl rounded-2xl p-5 border transition-all duration-300 ${theme.wrapper}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white text-lg tracking-wide">{att.userId?.name}</h3>
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-wider font-semibold">{att.userId?.role}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4 border-t border-slate-200 dark:border-slate-800/50">
                    <div>
                      ...
                      {att.punchIn?.time ? (
                        <div>
                          <div className="text-sm font-bold text-slate-900 dark:text-white">{new Date(att.punchIn.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                    ...
                    <div>
                      ...
                      {att.punchOut?.time ? (
                        <div>
                          <div className="text-sm font-bold text-slate-900 dark:text-white">{new Date(att.punchOut.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                  ...
                  <div className="pt-4 border-t border-slate-200 dark:border-slate-800/50">
                    <button onClick={() => openEditModal(att)} className="w-full py-2.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-blue-500/20 hover:border-blue-500/50 hover:text-blue-600 dark:hover:text-blue-400 text-slate-600 dark:text-slate-300 rounded-xl transition-all flex items-center justify-center gap-2 text-sm font-bold shadow-sm">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#0B1121] border border-slate-200 dark:border-slate-700/50 rounded-3xl w-full max-w-xl overflow-hidden shadow-md dark:shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col">
            <div className="p-5 border-b border-slate-200 dark:border-slate-800/80 flex justify-between items-center bg-slate-50 dark:bg-slate-900/40">
              <h3 className="font-bold flex items-center text-slate-900 dark:text-white tracking-wide"><Map className="mr-2 h-5 w-5 text-blue-500" /> Location: {selectedMap.name}</h3>
              <button onClick={() => setSelectedMap(null)} className="p-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-red-500/20 text-slate-550 dark:text-slate-400 hover:text-red-400 transition-colors"><X className="h-5 w-5"/></button>
            </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in zoom-in-95 duration-200">
          <div className="bg-white dark:bg-[#0B1121] rounded-3xl w-full max-w-sm p-6 shadow-xl dark:shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-slate-200 dark:border-slate-700/50">
             <div className="flex justify-between items-center mb-6">
               ...
               <button onClick={() => setEditingRecord(null)} className="p-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-red-500/20 text-slate-550 dark:text-slate-400 hover:text-red-400 transition-colors"><X className="h-5 w-5"/></button>
             </div>

            <div className="space-y-5 mb-6">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5">Date Target</label>
                <input type="date" className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900/50 text-slate-800 dark:text-slate-200 text-sm outline-none focus:border-blue-500 [color-scheme:dark] transition-colors"
                  value={editForm.date}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="time" className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900/50 text-slate-800 dark:text-slate-200 text-sm outline-none focus:border-blue-500 [color-scheme:dark] transition-colors"
              ...
              <div>
                ...
                <input type="text" placeholder="Reason for override..."
                  ...
                  className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-900/50 placeholder:text-slate-400 dark:placeholder:text-slate-600 text-slate-800 dark:text-slate-200 outline-none text-sm focus:border-blue-500 transition-colors" />
              </div>
            </div>
