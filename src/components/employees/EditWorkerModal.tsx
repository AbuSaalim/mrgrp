...
export default function EditWorkerModal({ isOpen, onClose, onWorkerUpdated, worker }: EditWorkerModalProps) {
  ...
  useEffect(() => {
    if (worker && isOpen) {
      setFormData({
        name: worker.name || "",
        skill: worker.skill || "",
        perHourRate: worker.perHourRate ? worker.perHourRate.toString() : "",
      });
      setError("");
    }
  }, [worker, isOpen]);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-800">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Edit Worker Details</h2>
          <button
            onClick={onClose}
